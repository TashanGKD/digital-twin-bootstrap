...
function RadarChart({ title, dimensions, values, maxValue }: ChartBlockProps) {
  const size = 280
  const center = size / 2
  const radius = 110
  const n = dimensions.length
  if (n < 3) return <BarChart title={title} dimensions={dimensions} values={values} maxValue={maxValue!} chartType="bar" />
...
function BarChart({ title, dimensions, values, maxValue }: ChartBlockProps) {
  return (
    <div className="block-chart">
      <h4 className="chart-title">{title}</h4>
