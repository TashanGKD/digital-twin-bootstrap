  ...
  return (
    <div className="sci-scatter-wrap">
      <svg viewBox={`0 0 ${W} ${H}`} className="sci-scatter-svg">
        ...
        {/* 轴标签 */}
        <text x={W / 2} y={H - 8} fontSize="10" fill="#6b7280" textAnchor="middle">认知风格 (CSI: 深度型 ← → 整合型)</text>
        ...
        {scatterData.map((p, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={px} y={py - 8} fontSize="8" fill="#374151" textAnchor="middle">{p.name.length > 4 ? p.name.slice(0, 4) + '..' : p.name}</text>
            </g>
          )
        })}
        ...
        <text x={ux} y={uy - 14} fontSize="12" fill="#000" textAnchor="middle" fontWeight="700">你</text>
      </svg>
    </div>
  )
}
