...
- **新建科研数字分身**：用户在 collect-basic-info 流程中选择了「从其他AI迁移」
- **已有数字分身**：用户主动说「从 AI 记忆导入」等

## 步骤一：判断画像状态

**不询问用户使用哪个 AI 工具**。提示词对所有 AI 平台通用。

调用 read_profile，判断哪些维度为空白：
...
- **已有部分数据**：仅针对空白维度生成「补充提示词」

## 步骤二：生成结构化提示词

**输出格式要求（必须严格遵守）**：

1. 先用普通文字写一句引导语：「请点击下方「一键复制」按钮，将提示词粘贴到你所使用的 AI 的对话框中发送。」
2. 然后调用 **show_copyable** 工具，将完整提示词作为 content 参数传入。title 设为「科研数字分身信息提取提示词」。
3. 提示词内容必须**原样使用以下模板**，不要自行改写措辞。根据步骤一的判断，将需要的模块组合成一段完整文本传入 show_copyable。
...
使用建议：
1. 直接粘贴到对话框发送即可，无需修改
2. 如果你使用多个 AI 工具，请将同一提示词分别发送给每个 AI，然后把各 AI 的回复依次粘贴回来，我会帮你整合
3. 如果 AI 回复"记忆不足"较多，可以追问：「你记得我跟你聊过关于[具体话题]的事情吗？」
4. 拿到 AI 的回复后，回来把内容粘贴给我
...
紧接在 show_copyable 之后，调用 ask_text：
- question: "复制提示词并发给你的 AI 后，请将 AI 的回复粘贴到下方。\n如果你发给了多个 AI，可以把所有回复依次粘贴在一起，我会帮你整合。\n\n如果不想走迁移路径，可以直接输入「跳过」。"
- placeholder: "将 AI 的回复粘贴到这里（支持多个 AI 的回复一起粘贴）..."
- multiline: true

收到用户粘贴的内容后：
- 若内容为「跳过」或类似表达 → 回到 collect-basic-info 的第二步（逐项采集基础身份）
- 否则 → 调用 read_skill("import-ai-memory") 处理粘贴的内容

## 提示词安全原则

- 只询问与画像维度直接相关的信息
- **严禁**在提示词中涉及财务、健康、家庭关系、政治观点等隐私内容
- **不询问用户使用哪个 AI 工具**，提示词对所有 AI 平台通用
